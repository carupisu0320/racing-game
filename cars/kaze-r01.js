...
function buildKazeR01(paintColorHex) {
  ...
  const bodyStations = [
    { z:  2.26, halfWidth: 0.07, bottomY: groundClearance, topY: 0.31 },
    { z:  2.12, halfWidth: 0.32, bottomY: groundClearance, topY: 0.34 },
    { z:  1.92, halfWidth: 0.58, bottomY: groundClearance, topY: 0.39 },
    { z:  1.62, halfWidth: 0.75, bottomY: groundClearance, topY: 0.45 },
  ...
  // --------------------------------------------------------
  // 8. Front fender openings
  // --------------------------------------------------------
  // Small dark gaps above the tires help visually separate the
  // wheel from the body without drawing a large black outline.
  [-1, 1].forEach(sign => {
    const gap = new THREE.Mesh(
      new THREE.TorusGeometry(0.345, 0.018, 5, 18, Math.PI * 0.90),
      blackMat
    );
    gap.rotation.z = Math.PI / 2;
    gap.rotation.y = Math.PI;
    gap.position.set(sign * halfTrackFront, 0.335, frontAxleZ);
    car.add(gap);
  });
